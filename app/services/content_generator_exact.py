"""
Content generation service with exact Zapier OpenAI prompts.

This implements the exact prompts from the Zapier workflow:
- Step 25: Company Description (gpt-3.5-turbo-instruct, temp=0.7)
- Step 26: KB Articles (gpt-3.5-turbo-instruct, temp=0.5)
- Step 28: 70 Questions (gpt-3.5-turbo-instruct, temp=0.7)
- Step 30: Beeceptor Endpoints (gpt-3.5-turbo-instruct, temp=0.7)
"""
...
class ContentGeneratorExact:
    """Content generator with exact Zapier prompts."""
...
Now generate and return ONLY this JSON array with objects 1 through 10 filled in."""
        ...
        response = await self.openai.generate_completion(
            ...
            model="gpt-3.5-turbo-instruct",
            temperature=0.5,
            max_tokens=4000  # Need more tokens for 10 articles
        )

        # Parse JSON
        try:
            articles = json.loads(response.strip())
...
No text outside the JSON. No markdown formatting."""
        ...
        # Parse JSON
        try:
            questions_obj = json.loads(response.strip())
